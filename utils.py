...
def preprocess_example(data, data_type="train"):
    ...
    for waveform, _, _, utterance, _, _, _ in data:
        ...
        if data_type == 'train':
            spec = train_audio_transform(waveform).squeeze(0).transpose(0, 1) # (time, freq)
        else:
            spec = valid_audio_transform(waveform).squeeze(0).transpose(0, 1) # (time, freq)
        ...
        input_lengths.append(spec.shape[0])     # account for subsampling of time dimension
